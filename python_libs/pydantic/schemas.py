...
class PaymentsFile(BaseModel):
    ...
    raw_hash_digest: str = Field(
        description="Hash digest for the file content as it was read from Kafka."
    ...
    kafka_group: str = Field(
        description="Kafka group used to consume the messages.", max_length=64
    )
    processed_hash_digest: Optional[str] = Field(
        description="Hash digest for the file content after"
        " being ordered and cleaned. Use hash_file function.",
        max_length=96,
        min_length=96,
    )
    datadog_start_date: Optional[datetime] = Field(
        description="Date the file started to be sent to Datadog."
    )
    datadog_end_date: Optional[datetime] = Field(
        description="Date the file finished being sent to Datadog."
    )
    data_dog_count: Optional[int] = Field(
        description="Amount of records sent to Datadog.", ge=0
    )
